...
export default function App() {
  ...
  return (
    <div className={`${theme === 'dark' ? 'dark' : ''}`}>
      <div className="main-container bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white min-h-screen w-full flex flex-col items-center justify-center font-sans selection:bg-cyan-500 selection:text-white transition-colors duration-300">
        <ThemeToggle />
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 items-start flex-1 px-2 sm:px-4">
        {/* Options Panel */}
          ...
          <form onSubmit={handleAddOption} className="flex flex-wrap gap-2 mb-4">
            <input
              type="text"
              value={newOption}
              onChange={(e) => setNewOption(e.target.value)}
              placeholder="Add an option..."
                className="flex-grow min-w-0 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-cyan-500 transition border border-gray-300 dark:border-gray-600"
            />
